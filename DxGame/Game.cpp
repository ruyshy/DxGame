...
void Game::Update()
{
	...
	_deviceContext->Map(_contantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &subResource);
	memcpy(subResource.pData, &_transformData, sizeof(_transformData));
	_deviceContext->Unmap(_contantBuffer.Get(), 0);
}
...
void Game::Render()
{
	RenderBegin();
	{
		...
		_deviceContext->VSSetConstantBuffers(0, 1, _contantBuffer.GetAddressOf());
...
void Game::CreateConstantBuffer()
{
	...
	HRESULT hr = _device->CreateBuffer(&desc, nullptr, _contantBuffer.GetAddressOf());
	CHECK(hr);
}
